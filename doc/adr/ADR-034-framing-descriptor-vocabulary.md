...
## Context
ADR-018 settled that framing is data-driven from the descriptor's `:framing` block (a generic framer at the connection), not per-protocol code — but left the *vocabulary* of `:framing` undefined. The framer must recover packet structure from raw stream bytes — delimit by length, verify CRC, de-obfuscate, emit a parsed packet — parameterized only by the descriptor, so the CubeWiFi protocol (and any length-prefixed + CRC + XOR peer) needs no framing code. This ADR defines that vocabulary.
...
`{:counts [...]}` names the components the length field sums. `payload_len = length − Σ(widths of non-:payload counts)`. For CubeWiFi `:counts [:unit :type :payload]` ⇒ `payload_len = length − 1 − 1 = length − 2`. Self-documenting — the `−2` is never a bare magic constant; it falls out of the named header widths. Total packet size = `Σheader widths + payload_len + :crc :width`.
...
`{:when {<field> <value>} :algo :xor-repeating :key ... :applies-to :payload}`. The framer de-obfuscates only when the named header field equals `:when`'s value (CubeWiFi: `proto == 0x0006`). `:applies-to :payload` — the header is plaintext. Header obfuscation is not in the vocabulary (defer to the escape-hatch if ever needed).
...
A `:framer-fn` key (a per-protocol fn taking raw stream → parsed packets) is *deferred*, mirroring `:derive`'s escape-hatch: not added until a real protocol's framing the declarative vocabulary can't express. CubeWiFi needs none.

### Placement
The framer is a generic component at the **connection** (the stream owner); the connection drives it and routes its output (ADR-018 / TDD-01). It is protocol-agnostic — protocol knowledge lives only in the `:framing` data.
...
## Consequences
- (+) The framer is fully data-driven; CubeWiFi needs no framing code — `:framing` transcribes the protocol-doc framing section.
...
- (-) The vocabulary is bounded — a protocol outside the length-prefixed + CRC + payload-XOR family needs the deferred `:framer-fn`. Acceptable: the escape-hatch mirrors `:derive`; CubeWiFi and likely peers are covered.
